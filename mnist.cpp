...
int main(int argc, char ** argv) {
    ...
    std::string model_fname = "/Users/yifengyu/hack/models/mnist/ggml-model-f32.bin";
    std::string t10k_image = "/Users/yifengyu/hack/models/mnist/t10k-images.idx3-ubyte";
    ...
    // load the model, load a random test digit, evaluate the model
    {
        ...
        if (!mnist_model_load(model_fname, model)) {
            fprintf(stderr, "%s: failed to load model from '%s'\n", __func__, "models/ggml-model-f32.bin");
            return 1;
        }
        auto fin = std::ifstream(t10k_image, std::ios::binary);
        if (!fin) {
            fprintf(stderr, "%s: failed to open '%s'\n", __func__, argv[2]);
            return 1;
        }
